...
public class Collections3 {
	...
	/**
	 * 返回id集合，同时把exists里面，inputs当中没有的移除
	 *
	 * @param inputs
	 * @param exists
	 * @param property
	 * @return
	 */
	public static <T> Set<T> mergeIds(Collection inputs, Collection exists, String property, Class<T> clazz) {
		Set<T> newIds = new TreeSet<>();
		Set<T> inputsIds = new TreeSet<>();
		Set<T> existsIds = new TreeSet<>();

		//先移除
		for (Object o : inputs) {
			T id = (T)BeanUtils.getProperty(o, property);
			if (id != null) {
				inputsIds.add(id);
			}
		}
